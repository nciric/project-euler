...
// An optimization would be to not multiply all 13 digits every time, but to
// divide with leading and multiply with next in range. We would need to pay
// attention to division by 0, and would need to re-calculate if result is 0.
// This is similar to substring matching algorithm.

const int kDigits = 13;

unsigned long Multiply(const int* data, int index) {
...
unsigned long FindSolution() {
  constexpr std::array<int, 1000> digits = {
  ...
  for (int i = 0; i < digits.size() - kDigits; ++i) {
    unsigned long result = Multiply(digits.data(), i);
